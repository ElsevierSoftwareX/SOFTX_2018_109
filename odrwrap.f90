      ...
      subroutine odr_wrap( n,m,np,nq,x,y,beta, ifix, &
      ...
      ! set non-default  stuff
      lowerbound(1)= 0
      lowerbound(2)= 0
      lowerbound(3)= 1
      upperbound(1)= huge(zero)
      upperbound(2)= maxval(x)
      upperbound(3)= 2
...
!                    covariance(2,3)/stdev_beta(2)/stdev_beta(3)
      ...
      real(kind=r8) function fun(x,a,hp,m)

      implicit none
      real(kind=r8), intent(in) :: x,a,hp,m
      ...
      real(kind=r8) function dfun_da(x,a,hp,m)
      ...
      real(kind=r8) function dfun_dm(x,a,hp,m)

      implicit none
      real(kind=r8), intent(in) :: x,a,hp,m

      dfun_dm= a*pow(x-hp,m)*log(x-hp)

      end function dfun_dm





      real(kind=r8) function dfun_dhp(x,a,hp,m)

      implicit none
      real(kind=r8), intent(in) :: x,a,hp,m

      dfun_dhp= -a*m*pow(x-hp,m-1)

      end function dfun_dhp



      real(kind=r8) function dfun_ddeltai(x,a,hp,m)

      implicit none
      real(kind=r8), intent(in) :: x,a,hp,m

      dfun_ddeltai= a*m*pow(x-hp,m-1)
      ...
      if (mod(ideval,10) .ge. 1) then
              do l=1,nq
                 do i=1,n
                     f(i,l) = fun(xplusd(i,l), beta(1),beta(2), beta(3))
                 end do
              end do    
      end if         
      if (mod(ideval/10,10) .ge. 1) then
              do l=1,nq
                 do i=1,n
                     fjacb(i,1,l) = dfun_da(xplusd(i,l), beta(1),beta(2), beta(3))
                     fjacb(i,2,l) = dfun_dhp(xplusd(i,l), beta(1),beta(2), beta(3))
                     fjacb(i,3,l) = dfun_dm(xplusd(i,l), beta(1),beta(2), beta(3))
                 end do
              end do    
      end if         
      if (mod(ideval/100,10) .ge. 1) then
              do l=1,nq
                 do j=1,m
                     do i=1,n
                         fjacd(i,j,l) = dfun_ddeltai(xplusd(i,j), beta(1),beta(2), beta(3))
                     end do
                 end do
              end do    
      end if         
